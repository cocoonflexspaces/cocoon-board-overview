#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# deploy.sh — push the Cocoon Board Overview deck to GitHub Pages
#
# What this does:
#   1. Initializes a git repo in this folder (if not already)
#   2. Commits everything (respecting .gitignore)
#   3. Creates the GitHub repo (or reuses an existing one)
#   4. Pushes the deck
#   5. Makes the repo public + enables GitHub Pages on the main branch
...
DEFAULT_REPO_NAME="cocoon-board-overview"
...
if ! git diff --cached --quiet; then
  git commit -q -m "Update board overview deck"
fi

# --- 4. Create remote repo (private by default; flip to --public if you want) ---
...
# Make sure remote is set if repo pre-existed
